...
<body>
	<div class="main">
    	<div class="pos">修改菜单</div>
		<div class="form">
			<form method='post' id="form_do" name="form_do" action="$link.setAction('modifyMenu')">
				<div id="div_setting_1">
						...
						<dd>
							<select name="menu.pid">
								...
								#foreach($menu in $menuList)
									#if($!menu.id==$!modifyMenuForm.menu.pid)
					...
					<dl>
						<dt> 菜单名称：</dt>
						<dd>
							<input type="text" name="menu.name" class="inp_large" value="$!modifyMenuForm.menu.name" />
					...
					<dl>
						<dt>路径：</dt>
						<dd>
							<input type="text" name="menu.url" class="inp_large" value="$!modifyMenuForm.menu.url" />
						</dd>
					</dl>		
					<dl>
						<dt> 排序：</dt>
						<dd>
							<input type="text" name="menu.sort" class="inp_one" value="$!modifyMenuForm.menu.sort" />
						</dd>
					</dl>
					<dl>
						<dt> 显示：</dt>
						<dd>
							<label><input type="radio" name="menu.isshow" value="1" #if($!modifyMenuForm.menu.isshow==1) checked="checked" #end />显示&nbsp;</label>
							<label><input type="radio" name="menu.isshow" value="0" #if($!modifyMenuForm.menu.isshow==0) checked="checked" #end />隐藏</label>
						</dd>
					</dl>
				</div>
				<div class="form_b">
					<input type="hidden" name="menu.id" value="$!modifyMenuForm.menu.id" />
