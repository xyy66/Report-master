...
<body>
<div class="main">
    <div class="pos">修改新闻</div>
	<div class="form">
		<form method='post' id="form_do" name="form_do" action="$link.setAction('modifyNews')">
		<dl>
			<dt> 新闻标题：</dt>
			<dd>
				<input type="text" name="news.title" class="inp_large" value="$!modifyNewsForm.news.title"/>
		...
		<dl>
			<dt> 标题颜色：</dt>
			<dd>
				<input type="text" name="news.color" id="color" class="inp_small iColorPicker" readonly="readonly" value="$!modifyNewsForm.news.color"/>
			</dd>
		</dl>
		<dl>
			<dt> 关键词：</dt>
			<dd>
				<input type="text" name="news.keywords" class="inp_w250" value="$!modifyNewsForm.news.keywords"/><span class="tip">多关键词之间用“,”隔开</span>
			</dd>
		</dl>
		<dl>
			<dt> 新闻摘要：</dt>
			<dd>
				<textarea name="news.description" id="description" class="tarea_default">$!modifyNewsForm.news.description</textarea>
			</dd>
		</dl>
		<dl>
			<dt> 作者：</dt>
			<dd>
				<input type="text" name="news.author" class="inp_w250" value="$!modifyNewsForm.news.author"/><span class="tip"></span>
			</dd>
		</dl>
		<dl>
			<dt> 新闻内容：</dt>
			<dd>
				<textarea name="news.content" id="content">$!modifyNewsForm.news.content</textarea>
			</dd>
		</dl>		
		<dl>
			<dt> 是否发布：</dt>
			<dd>
				<label><input type="radio" name="news.ispublish" value="1" #if($!modifyNewsForm.news.ispublish==1) checked="checked" #end/>立即发布&nbsp;</label>
				<label><input type="radio" name="news.ispublish" value="0" #if($!modifyNewsForm.news.ispublish==0) checked="checked" #end/>暂不发布</label>
			</dd>
		</dl>
		<dl>
            <dt> 发布时间：</dt>
            <dd>
                <input type="text" class="inp_one" name="news.publishtime" id="publishtime" readonly="readonly" value="$!modifyNewsForm.news.publishtime" />
				#if($!modifyNewsForm.news.ispublish==1) <span style="color: red;">*</span> #end
                <script type="text/javascript">
                    Calendar.setup({
                        weekNumbers: true,
		...
		<div class="form_b">		
			<input type="hidden" name="news.id" value="$!modifyNewsForm.news.id" />
